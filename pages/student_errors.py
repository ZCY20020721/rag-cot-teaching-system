...
def page_student_errors():
    ...
    if error_stats:
        ...
        fig = px.bar(df, x="tag", y="count", title="个人薄弱知识点分布",
    ...
    weak_records = [
        r for r in records
        if (r["weak_tags"] and json.loads(
            r["weak_tags"] if isinstance(r["weak_tags"], str) else "[]"
        ))
    ]
    if weak_records:
        for r in weak_records:
            weak = json.loads(r["weak_tags"]) if isinstance(r["weak_tags"], str) else (
                r["weak_tags"] or []
            )
            with st.expander(
                f"[{r['created_at']}] {'、'.join(weak)} — {r['question'][:50]}..."
            ):
